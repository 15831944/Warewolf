...
namespace Dev2.Infrastructure.Tests
{
    ...
    public static class TestContextExtentionMethod
    {
        public static Container GetContainer(this Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testCtx)
        {
            try
            {
                return ((Depends) Type.GetType(testCtx.FullyQualifiedTestClassName).GetMethod(testCtx.TestName)
                    .GetCustomAttributes(typeof(Depends), false)[0]).Container;
            }
        ...
        public static void DisposeContainers(this Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testCtx)
        {
            try
            {
                ((Depends) Type.GetType(testCtx.FullyQualifiedTestClassName).GetMethod(testCtx.TestName)
                    .GetCustomAttributes(typeof(Depends), false)[0]).Dispose();
            }
            catch (SystemException)
            {
                //Already disposed
